#Code to simulate a network with a mesh topology of 28 nodes using the Link State (LS) protocol
#By Drew Tomkins

#Create a simulator object
set ns [new Simulator]

#Make it so the simulator uses the Distance Vector protocol to enable dynamic routing
$ns rtproto LS
...
set nf [open Scenario_28_nodes_LS.nam w]
$ns namtrace-all $nf

#Open the trace file
set tf [open Scenario_28_nodes_LS.tr w]
$ns trace-all $tf

#Use a for loop to set up twenty-eight nodes
...
for {set i 0} {$i < 28} {incr i} {
       #Ensure that there are only up to 28 nodes in the topology
        for {set j [expr $i+1]} {$j < 28} {incr j} {
...
#Sets the TCP connection up with FTP
...
#Create a Null agent (a traffic sink) and attach it to node n(17)
set null0 [new Agent/Null]
$ns attach-agent $n(17) $null0
...
# Create a CBR traffic source with a rate of 2mb + packet size of 1000 and attach it to udp0
...
proc finish {} {
	...
	#Execute nam on the trace file
        exec nam Scenario_28_nodes_LS.nam &
        exit 0
}
